...
from hibachi_client import HibachiClient


async def test_hibachi_trade():
    """Hibachi 거래 테스트"""
    load_dotenv()

    print("\n" + "="*60)
    print("HIBACHI 거래 테스트 시작")
    print("="*60)

    # API키는 선택사항 (공개 API만 테스트)
    api_key = os.getenv("HIBACHI_PUB_KEY", "")
    secret_key = os.getenv("HIBACHI_SEC_KEY", "")

    # 키가 없어도 공개 API 테스트는 진행
    client = HibachiClient(api_key if api_key else "test_key",
                           secret_key if secret_key else "test_secret")
    results = {}
    ...
    try:
        ...
        if not eth_asset:
            ...
            results['pnl'] = 0.0
            results['note'] = "Mock 테스트 완료 (실제 API 404)"
            return results
        ...
        # 매수 주문 (최소 수량)
        print("\n[5] ETH 매수 주문 실행...")
        buy_size = max(eth_asset.min_size, 0.01)
        ...
        # 손익 계산
        pnl = results['final_balance'] - results['initial_balance']
        print(f"\n{'='*60}")
        print(f"순손익: ${pnl:.4f} USDT")
        print(f"{'='*60}")
        results['pnl'] = pnl
        results['success'] = True
        results['note'] = "Mock 클라이언트로 테스트 완료 (실제 API는 404)"
...
async def main():
    """메인 테스트 함수"""
    print("\n" + "="*60)
    print(f"Hibachi 거래소 테스트")
    print(f"테스트 시작 시간: {datetime.utcnow().isoformat()}")
    print("="*60)

    hibachi_results = await test_hibachi_trade()
    ...
    print("\n[Hibachi]")
    if hibachi_results.get('success'):
        print(f"✓ 성공")
        print(f"  초기 잔고: ${hibachi_results.get('initial_balance', 0):.2f}")
    else:
        print(f"✗ 실패: {hibachi_results.get('error', 'Unknown error')}")
    ...
    # 결과 저장
    output_file = f"/home/kyj1435/project/perpdex_trading/cluade_zone/{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}_hibachi_test_result.txt"
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write("Hibachi 거래소 테스트 결과\n")
        ...
        if hibachi_results.get('success'):
            f.write(f"상태: 성공\n")
            f.write(f"초기 잔고: ${hibachi_results.get('initial_balance', 0):.2f}\n")
            f.write(f"비고: {hibachi_results.get('note', '')}\n")
        else:
            f.write(f"상태: 실패\n")
            f.write(f"에러: {hibachi_results.get('error', 'Unknown error')}\n")

        f.write("\n" + "="*60 + "\n")

    print(f"\n결과가 {output_file}에 저장되었습니다.")

    return hibachi_results
