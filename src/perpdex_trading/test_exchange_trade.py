...
async def test_backpack_trade():
    """Backpack 거래 테스트"""
    load_dotenv()

    print("\n" + "="*60)
    print("BACKPACK 거래 테스트 시작")
    print("="*60)
    ...
    if not api_key or not secret_key:
        return {"error": "Backpack API 키가 설정되지 않았습니다"}
    ...
    try:
        ...
        eth_asset = None
        for asset in assets:
            if 'ETH' in asset.symbol and ('PERP' in asset.symbol or 'USDT' in asset.symbol):
                eth_asset = asset
                break
...
async def test_grvt_trade():
    """GRVT 거래 테스트"""
    load_dotenv()

    print("\n" + "="*60)
    print("GRVT 거래 테스트 시작")
    print("="*60)
    ...
    if not api_key or not private_key:
        return {"error": "GRVT API 키가 설정되지 않았습니다"}
...
async def main():
    """메인 테스트 함수"""
    print("\n" + "="*60)
    print(f"거래소 매수/매도 테스트")
    print(f"테스트 시작 시간: {datetime.utcnow().isoformat()}")
    print("="*60)

    # Backpack 테스트
    backpack_results = await test_backpack_trade()

    # 잠시 대기
    await asyncio.sleep(3)

    # GRVT 테스트
    grvt_results = await test_grvt_trade()
    ...
    print("\n[Backpack]")
    if backpack_results.get('success'):
        print(f"✓ 성공")
        print(f"  초기 잔고: ${backpack_results['initial_balance']:.2f}")
        print(f"  최종 잔고: ${backpack_results['final_balance']:.2f}")
        print(f"  순손익: ${backpack_results['pnl']:.4f}")
    else:
        print(f"✗ 실패: {backpack_results.get('error', 'Unknown error')}")

    print("\n[GRVT]")
    if grvt_results.get('success'):
        print(f"✓ 성공")
        print(f"  초기 잔고: ${grvt_results['initial_balance']:.2f}")
        print(f"  최종 잔고: ${grvt_results['final_balance']:.2f}")
        print(f"  순손익: ${grvt_results['pnl']:.4f}")
    else:
        print(f"✗ 실패: {grvt_results.get('error', 'Unknown error')}")
    ...
    # 결과를 파일로 저장
    output_file = f"/home/kyj1435/project/perpdex_trading/cluade_zone/{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}_trade_test_result.txt"
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write("거래소 매수/매도 테스트 결과\n")
        f.write(f"테스트 시간: {datetime.utcnow().isoformat()}\n")
        f.write("="*60 + "\n\n")

        f.write("[Backpack 테스트]\n")
        if backpack_results.get('success'):
            f.write(f"상태: 성공\n")
            f.write(f"거래 심볼: {backpack_results.get('symbol', 'N/A')}\n")
            f.write(f"현재가: ${backpack_results.get('current_price', 0):.2f}\n")
            f.write(f"초기 잔고: ${backpack_results['initial_balance']:.2f}\n")
            f.write(f"최종 잔고: ${backpack_results['final_balance']:.2f}\n")
            f.write(f"순손익: ${backpack_results['pnl']:.4f}\n")
            f.write(f"매수 체결가: ${backpack_results.get('buy_filled_price', 0):.2f}\n")
            f.write(f"매도 체결가: ${backpack_results.get('sell_filled_price', 0):.2f}\n")
        else:
            f.write(f"상태: 실패\n")
            f.write(f"에러: {backpack_results.get('error', 'Unknown error')}\n")
        ...
        if grvt_results.get('success'):
            f.write(f"상태: 성공\n")
            f.write(f"거래 심볼: {grvt_results.get('symbol', 'N/A')}\n")
            f.write(f"현재가: ${grvt_results.get('current_price', 0):.2f}\n")
            f.write(f"초기 잔고: ${grvt_results['initial_balance']:.2f}\n")
            f.write(f"최종 잔고: ${grvt_results['final_balance']:.2f}\n")
            f.write(f"순손익: ${grvt_results['pnl']:.4f}\n")
            f.write(f"매수 체결가: ${grvt_results.get('buy_filled_price', 0):.2f}\n")
            f.write(f"매도 체결가: ${grvt_results.get('sell_filled_price', 0):.2f}\n")
        else:
            f.write(f"상태: 실패\n")
            f.write(f"에러: {grvt_results.get('error', 'Unknown error')}\n")
